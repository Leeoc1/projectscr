...
const ReservationSeatPage = () => {
  ...
  // 선택된 좌석이 변경될 때마다 selectedMovieTime에 seatcd를 추가
  useEffect(() => {
    // 기존 selectedMovieTime 불러오기
    const movieTimeRaw = sessionStorage.getItem("selectedMovieTime");
    if (movieTimeRaw) {
      const movieTime = JSON.parse(movieTimeRaw);
      movieTime.seatcd = selectedSeats; // seatcd 배열 추가
      sessionStorage.setItem("selectedMovieTime", JSON.stringify(movieTime));
    }
  }, [selectedSeats]);
