...
SELECT
    CONCAT('SCH', LPAD((m.idx - 1) * 20 + t.n, 4, '0')),
    m.moviecd,
    (SELECT screencd FROM screen ORDER BY RAND() LIMIT 1),
    DATE_ADD(CURRENT_DATE(), INTERVAL FLOOR(RAND() * 7) DAY),
    SEC_TO_TIME(FLOOR(RAND() * (12 * 3600)) + (8 * 3600)),
    ADDTIME(
        SEC_TO_TIME(FLOOR(RAND() * (12 * 3600)) + (8 * 3600)),
        SEC_TO_TIME(FLOOR(RAND() * 61 + 90) * 60 + 30 * 60)
    )
FROM (SELECT moviecd, ROW_NUMBER() OVER () AS idx FROM movie LIMIT 200) m
    CROSS JOIN temp_nums t;
