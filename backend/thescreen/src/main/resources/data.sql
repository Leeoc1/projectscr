...
SELECT 'CONFIRMED' UNION SELECT 'PENDING' UNION SELECT 'CANCELLED';
...
SELECT
    ...
    (SELECT status FROM temp_status ORDER BY RAND() LIMIT 1) AS reservationstatus,
...
DROP TEMPORARY TABLE temp_status;
...
SET
    description = CASE
                      ...
                      WHEN movienm = '드래곤 길들이기' THEN '수백년간 지속되어온 바이킹과 드래곤의 전쟁.드래곤을 없애는 것이 삶의 모든 목적인 바이킹들과 다른 신념을 가진 히컵은 무리 속에 속하지 못하고 족장인 아버지에게도 인정받지 못한다.그러던 어느 날,히컵은 베일에 싸인 전설의 드래곤 나이트 퓨어리인 투슬리스와 만나게 되고, 드래곤을 죽이라는 바이킹의 신념을 깨고 투슬리스와 친구가 된다.하지만 드래곤을 죽여야 된다고 믿는 바이킹 족과 모든 드래곤을 위협하는 더 거대한 존재와 맞닥뜨리게 된 히컵과 투슬리스.세상을 변화시키기 위한 특별한 여정을 시작하게 되는데...다르다는 건, 특별하다는 것.세상을 바꿀 우리들의 모험이 시작된다!'
                      ...
                      WHEN movienm = '괴기열차' THEN '공포 실화를 찾아다니는 유튜브 크리에이터 다경(주현영)은 운영 중인 호러퀸 다경의 구독자 수가 줄자 더 열성적으로 무서운 이야기를 찾아 나선다. 목표는 미스터리한 사건이 벌어진다는 지하철 광림역이다. 광림역의 역장(전배수)은 꺼림칙한 듯 말을 아끼지만, 다경의 귀여운 술수에 넘어가 광림역의 숨겨진 이야기를 하나씩 털어놓기 시작한다. 이때부터 영화는 익숙한 공공장소에서 어디에나 있을법한 에피소드를 정교하게 설계해 나가며 기이한 폐쇄 공포를 일으킨다. 섬뜩한 실감과 장르적 쾌감을 증폭시켜 나가는 괴기열차 는 괴이한 이야기를 품고 느릿하게 때로는 숨 막히는 속도감으로 질주한다. 괴담의 짜릿한 유혹과 유머 그리고 호러물의 흥취를 안기는 흥미로운 대중 영화다. (홍은미)'
                      WHEN movienm = '슈퍼맨' THEN '세상의 희망인가, 위협인가?슈퍼맨은 오늘도 세계 곳곳의 위협에 맞서 싸우지만, 시민들의 반응은 극과 극으로 갈린다.한편, 렉스 루터는 슈퍼맨을 무너뜨릴 비밀을 손에 넣고 역대 최강의 슈퍼-빌런들과 함께 총 공격에 나선다.슈퍼맨은 첫 패배와 함께 이들의 계속된 공세에 직면하고 모든 것을 바로잡기 위해 슈퍼독 크립토와 함께 맞서게 되는데…과연 그는 이 전례 없는 위기에서 다시 날아오를 수 있을까?올여름, 가장 강력한 슈퍼히어로 블록버스터가 온다!'
                      ...
                      WHEN movienm = '신명' THEN '성형, 주술, 무당, 신분 위조까지…대한민국을 뒤흔든 그녀를 둘러싼 수많은 의혹들이 마침내 드러난다!어린 시절, 분신사바를 시작으로 주술에 심취한 윤지희(김규리 분). 남자를 이용해 원하는 것을 얻을 수 있다는 사실을 깨달은 그녀는 성형으로 얼굴을 바꾸기 시작해서 이름, 학력, 신분까지 위조해 전혀 다른 삶을 살아간다. 그러던 중 권력의 맛을 본 윤지희는 마침내 대한민국을 손에 넣겠다는 야망에 사로잡히고 필요하다면 주술로 사람의 목숨조차 앗아갈 만큼 잔혹한 행보를 이어간다.'
                      ...
                      WHEN movienm = '극장판 도라에몽: 진구의 그림이야기' THEN '만약, 그림 속 세계로 들어갈 수 있다면?환상의 보석을 둘러싼 도라에몽과 친구들의 시공 초월 대모험이 지금 시작된다!수백억 원의 가치를 지닌 그림이 발견되었다는 뉴스가 보도되는 가운데, 여름방학 숙제로 그림을 그리고 있던 진구 앞에 갑자기 오래된 그림 조각 하나가 떨어진다.도라에몽과 진구는 비밀도구 들어가는 라이트를 사용해 그림 속으로 탐험을 떠나고 그곳에서 신비로운 소녀 클레어와 만나게 된다.그런데 그곳은 바로 뉴스에서 화제가 된 그림 속에 그려진 중세 유럽 아트리아 공국이었다!그곳 어딘가에는 아트리아 블루라는 환상의 보석이 숨겨져 있다고 하는데…환상의 보석의 비밀을 파헤치는 도라에몽과 친구들.하지만, 아트리아 공국에 전해 내려오는 세계 멸망의 전설이 깨어나며 모두가 큰 위기에 빠진다!과연 도라에몽과 친구들은 전설을 뒤집고 세계를 구할 수 있을까?'
                      ELSE description
        END,
    posterurl = CASE
