...
SELECT
CONCAT('SCH', LPAD((m.idx - 1) * 10 + n, 4, '0')) AS schedulecd,
m.moviecd,
(SELECT screencd FROM screen ORDER BY RAND() LIMIT 1) AS screencd,
DATE_ADD(CURRENT_DATE(), INTERVAL FLOOR(RAND() * 7) DAY) AS startdate,      -- 오늘~7일 이내 랜덤
SEC_TO_TIME(FLOOR(RAND() * (12 * 3600)) + (8 * 3600)) AS starttime,          -- 08~20시 랜덤
ADDTIME(
SEC_TO_TIME(FLOOR(RAND() * (12 * 3600)) + (8 * 3600)),
SEC_TO_TIME(FLOOR(RAND() * 61 + 90) * 60 + 30 * 60)
) AS endtime
FROM
(SELECT moviecd, ROW_NUMBER() OVER () AS idx FROM movie LIMIT 200) m,
