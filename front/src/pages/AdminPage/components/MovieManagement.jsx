...
const MovieManagement = () => {
  ...
  const fetchMoviesFromKobisApi = async () => {
    if (!window.confirm("KOBIS API에서 최신 영화 데이터를 가져오시겠습니까?")) return;
    
    setIsLoading(true);
    try {
      const result = await fetchMoviesFromKobis();
      if (result.success) {
        setCurrentMovies(result.currentMovies || []);
  ...
  const archiveMovieItem = async (moviecd, movienm) => {
    if (!window.confirm(`"${movienm}"을 상영 종료하시겠습니까?\n(상영 종료된 영화는 웹 서비스에 더이상 노출되지 않습니다)`)) return;
  ...
  const getScreeningButtonText = (movieinfo) => {
    return movieinfo === "Y" ? "상영 종료" : "상영 시작";
  };

  const displayMovies = activeTab === "current" ? currentMovies : archivedMovies;
    ...
    <div className="adp-content">
      ...
      <div className="mvm-movie-management-tabs">
        ...
        <div className="mvm-tab-nav">
          <button
            className={`mvm-tab-btn ${activeTab === "current" ? "mvm-active" : ""}`}
            onClick={() => setActiveTab("current")}
          >
            현재 상영작 ({currentMovies.length})
          </button>
          <button
            className={`mvm-tab-btn ${activeTab === "archived" ? "mvm-active" : ""}`}
          ...
          {displayMovies.map((movie) => (
            <div className="mvm-movie-item" key={movie.moviecd}>
              ...
              <div className="mvm-movie-details">
                <h3>{movie.movienm}</h3>
                <p>장르: {movie.genre} | 등급: {getRating(movie.isadult)}</p>
                <p>상영시간: {movie.runningtime}분 | 개봉일: {movie.releasedate}</p>
                <p>감독: {movie.director}</p>
                <p>배우: {movie.actors}</p>
                <p>설명: {movie.description}</p>
                <p>상영상태: <span className={`status ${
                  movie.movieinfo === "Y" ? "screening" : 
                  movie.movieinfo === "N" ? "ready" : 
                  "archived"
                }`}>
                ...
                <div className="mvm-action-buttons">
                  {activeTab === "current" && (
                    <>
                      <button 
                        className={`mvm-screening-btn ${movie.movieinfo === "Y" ? "stop" : "start"}`}
                        onClick={() => toggleScreeningStatus(movie.moviecd, movie.movieinfo)}
                      >
                        {getScreeningButtonText(movie.movieinfo)}
                      </button>
                      <button 
                        className="mvm-archive-btn"
                        onClick={() => archiveMovieItem(movie.moviecd, movie.movienm)}
                      >
                        삭제
                      </button>
                    </>
                  ...
                  {activeTab === "archived" && (
                    <span className="mvm-archived-info">상영 종료된 영화입니다</span>
                  )}
                </div>
              </div>
            </div>
