...
import { getTotalVolume, getCinemaVolume, getAllUsers, getReservation } from "../../../../api/api";
...
const ChartSection = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [totalVolumeData, cinemaVolumeData, allUsersData, reservationData] = await Promise.all([
          getTotalVolume(),
          getCinemaVolume(),
          getAllUsers(),
          getReservation()
        ...
        const movieVolumeData = Object.entries(movieSales).map(([movieName, totalAmount]) => ({
          movieName,
          totalAmount
