  ...
  useEffect(() => {
    const fetchReservation = async () => {
      try {
        ...
        // 0으로 나누기 방지
        const increaseVolume = yesterdayTotalVolume > 0 
          ? ((todayTotalVolume - yesterdayTotalVolume) / yesterdayTotalVolume * 100).toFixed(2)
          : todayTotalVolume > 0 ? "100.00" : "0.00";

        //예매 건수 계산
        const todayReservationCount = todayReservation.length;
        const yesterdayReservationCount = yesterdayReservation.length;
        const increaseReservationCount = yesterdayReservationCount > 0
          ? ((todayReservationCount - yesterdayReservationCount) / yesterdayReservationCount * 100).toFixed(2)
          : todayReservationCount > 0 ? "100.00" : "0.00";

        //평균 객단가 계산
        const averagePrice = todayReservationCount > 0 ? todayTotalVolume / todayReservationCount : 0;
        const yesterdayAveragePrice = yesterdayReservationCount > 0 ? yesterdayTotalVolume / yesterdayReservationCount : 0;
        const increaseAveragePrice = yesterdayAveragePrice > 0
          ? ((averagePrice - yesterdayAveragePrice) / yesterdayAveragePrice * 100).toFixed(2)
          : averagePrice > 0 ? "100.00" : "0.00";
