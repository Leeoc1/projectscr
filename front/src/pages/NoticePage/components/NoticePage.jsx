...
import { fetchAllNotices, fetchAllFaqs, fetchAllReviews, getMoviesForAdmin } from '../../../api/api';
...
const NoticePage = () => {
  ...
  useEffect(() => {
    ...
    getMoviesForAdmin().then((movieData) => {
  ...
  // 전체 건수 표시 텍스트
  const totalCount = activeTab === 'notice' ? notices.length : activeTab === 'faq' ? faqs.length : reviews.length;
