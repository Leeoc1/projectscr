...
const ReservationSuccessPage = () => {
  ...
  const getMovieFromReservationInfo = () => {
    try {
      const savedReservationInfo = sessionStorage.getItem("finalReservationInfo");
    ...
    const fetchReservationData = async () => {
      try {
        ...
        if (data && data.length > 0) {
          ...
          // 예약 정보 조회 실패
          const savedReservationInfo = sessionStorage.getItem("finalReservationInfo");        if (savedReservationInfo) {
          const reservationInfo = JSON.parse(savedReservationInfo);
          const tempData = {
            reservationcd: "임시예약번호",
            seatcd: reservationInfo.selectedSeats || "",
