...
public interface ScheduleRepository extends JpaRepository<Schedule, String> {
    ...
    @Modifying
    @Transactional
    @Query(value = "DROP TEMPORARY TABLE IF EXISTS temp_nums", nativeQuery = true)
    void dropTempNums();

    // 임시 테이블 생성
    @Modifying
    @Transactional
    @Query(value = "CREATE TEMPORARY TABLE temp_nums (n INT)", nativeQuery = true)
    void createTempNumsTable();

    // 임시 테이블에 데이터 삽입 (5개 행)
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO temp_nums (n) SELECT ROW_NUMBER() OVER () - 1 AS N " +
